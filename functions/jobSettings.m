...
personalSettings.extPath = ['/home/', personalSettings.username];           % external path to your default directory. Use /home/username or /bigwork/username.
personalSettings.extUploadFolder = [personalSettings.extPath, '/myUploads'];% external (server) dir, where local files are uploaded to
personalSettings.locPath = pwd();                                           % path to local top level working dir
...
personalSettings.extDownloadFolder = [personalSettings.extPath, ...
    '/myUploads/$UPLOADDIR$/', personalSettings.relResultsFolder];          % external (server) dir, from which to download results, %UPLOADDIR% will be adjusted later by the program.
